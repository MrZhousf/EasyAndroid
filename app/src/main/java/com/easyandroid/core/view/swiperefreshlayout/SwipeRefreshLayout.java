...
package com.easyandroid.core.view.swiperefreshlayout;
 ...
 * instantiates this view should create an OnRefreshListener to be notified
...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    // Default offset in dips from the top of the view to wheres the progress spinner should stop
     ...
     *
     * @param scale Set to true if there is no view at a higher z-orders than
     *            wheres the progress spinner is set to appear.
    ...
    private void createProgressView() {
        mCircleView = new CircleImage(getContext(), CIRCLE_BG_LIGHT, CIRCLE_DIAMETER / 2);
...
//    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (mDirection) {
            case NONE://create by zhsf
                return false;
            case BOTTOM:
                if (!isEnabled() || mReturningToStart || (!mBothDirection && canChildScrollDown()) || mRefreshing) {
                    // Fail fast if we're not in a state wheres a swipe is possible
                    return false;
                }
                break;
            case TOP:
