...
public abstract class ApiSubscriber<T> extends Subscriber<T> {
    ...
    public void onError(Throwable e) {
        ...
        if(e instanceof HttpException){//对网络异常
            ...
            if(TextUtils.isEmpty(httpException.getMessage())){
                Util.Log.d(ApiSubscriber.class.getName(),"网络异常");
            }else {
                String errorMsg = httpException.getMessage();
                Util.Log.d(ApiSubscriber.class.getName(),""+errorMsg);
            }
            ...
            Util.Log.d(ApiSubscriber.class.getName(),"解析异常");
        }else if(e instanceof UnknownHostException){
            Util.Log.d(ApiSubscriber.class.getName(),"请求地址错误");
        }else if(e instanceof SocketTimeoutException) {
            Util.Log.d(ApiSubscriber.class.getName(),"请求超时");
        }else {//未知错误
            e.printStackTrace();
        }
    }
    ...
    protected  void onResultError(ApiException ex){
        Util.Log.d(ApiSubscriber.class.getName()," "+ex.getMessage());
        ex.printStackTrace();
    }
